...
Rainbow.extend('tamescript', [
    ...
    {
        name: 'command',
        pattern: /\b(NOOP|ADDCUE|TEXT|TEXT(F(LN)?|LN)|PAUSE|WAIT|AS(BOOLEAN|INT|FLOAT|STRING|LIST)|LENGTH|EMPTY|STR(LENGTH|CONCAT|REPLACE(LAST|ALL)?|INDEX|LASTINDEX|CONTAINS|STARTSWITH|ENDSWITH|LOWER|UPPER|CHAR|TRIM|PARAM|JOIN|SPLIT)|SUBSTRING|LIST(NEW|ADD(AT)?|REMOVE(AT)?|INDEX|CONTAINS|CONCAT)|ISREGEX|REGEX(CONTAINS|FIND(LAST)?|GET(LAST|ALL)?|MATCHES|SPLIT|REPLACE)|FLOOR|CEILING|ROUND|FIX|SQRT|PI|E|SIN|COS|TAN|MIN|MAX|CLAMP|IRANDOM|FRANDOM|GRANDOM|TIME(FORMAT)?|OBJECT(HAS(NAME|TAG|NOOWNER)?|COUNT)|ADDOBJECT(NAME|TAG(TOALLIN)?)|REMOVEOBJECT(NAME|TAG(FROMALLIN)?)?|GIVEOBJECT|MOVEOBJECTSWITHTAG|HASOBJECT|PLAYER(HASROOMINSTACK|CANACCESSOBJECT)|BROWSE(TAGGED)?|SET(PLAYER|ROOM)|PUSHROOM|POPROOM|SWAPROOM|CURRENT(PLAYERIS|ROOMIS)|NOCURRENT(PLAYER|ROOM)|IDENTITY|HEADER|ELEMENTHASANCESTOR)\b/gi
    },
    {
        name: 'entry',
